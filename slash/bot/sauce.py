...
class Sauce(commands.Cog):
    ...
    @cog_ext.cog_slash(name="sauce",
                    description="Find source of an image",
                    guild_ids=config.guild_ids,
                    options=[
                        create_option(name="url", 
                                    description="Url of an image or a Discord message. Leave blank to find a newest msg has image in 50 newest msgs.", 
                                    option_type=3, 
                                    required=False)
                    ]) # slash sauce command
    async def _sauce_(self, ctx: SlashContext, url:str = None):
        await self.replyLinks(ctx, url, saucenao=True, yandex=True, iqdb=True, iqdb3d=True, google=True, tineye=True, tracemoe=True, ascii2d=True)

    async def replyLinks(self, ctx, url, saucenao=False, yandex=False, iqdb=False, iqdb3d=False, google=False, tineye=False, tracemoe=False, ascii2d=False, no_preview=False):
        await ctx.defer()
        result = self.analyzeCommand(ctx, url)
        ...
        if result == "link":
            ...
            if len(urls) == 0:
                await ctx.send(content="Linked message does not have attached pictures.", hidden=True)
                return
            ...
            async for message in ctx.channel.history(limit=50):
                if message.attachments != [] or message.embeds != []:
                    ...
                    break
        if urls == []:
            await ctx.send(content=f"You have not provided anything to perform reverse search on.", hidden=True)
            return
        index = 1
        len_url = len(urls)
        if len_url > 1:
            await ctx.send(content=f"Found {len_url} images. Sending results...")
        for u in urls:
            ...
            try:
                ...
                if len(urls) > 1:
                    len_url = len_url - 1
                    await ctx.send(content=f"Found {len_url} images in linked message. Sending results...")
                    await ctx.send(e)
                    pass
                else:
                    await ctx.send(content=e, hidden=True)
                    return
            del embed
            index += 1
        if len(urls) > 1:
            await ctx.send(content=f"Found {len(urls)} images.")
        return

    def analyzeCommand(self, ctx, url):
        if url is None:
            return "history"
        else:
            if re.search(r"https://(ptb\.|canary\.){0,1}discord(app){0,1}\.com/channels/\d+/\d+/\d+", url, re.I) != None:
                return "discord link"
            else:
                return "link"
